...
jQuery.fn.motionCaptcha || (function($) {
	...
	$.fn.motionCaptcha.defaults = {
		...
		noCanvasMsg: "您的瀏覽器不提供 <canvas> - try Chrome, FF4, Safari or IE9.",
		// This could be any HTML string (eg. '<label>Draw this shit yo:</label>'):
		//label: '<p>Please draw the shape in the box to submit the form:</p>',
		label: '<p>請畫出畫面上的圖型才能登入:</p>',
		...
		onSuccess: function($form, $canvas, ctx) {
			...
			$submit = opts.submitId ? $form.find(opts.submitId) : $form.find('#login-btn');
			// Set the form action:
			$form.attr( 'action', $(opts.actionId).val() );
			
			// Enable the submit button:
			$submit.removeAttr('disabled');
			//$submit.removeData('data-disabled=true')
