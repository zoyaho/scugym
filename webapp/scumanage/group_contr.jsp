	...
	<article>
		<div class="table">
			<div class="table-head">
				...
				<div class="column" data-label="群組說明">群組說明</div>
				<div class="column" data-label="新增群組">
					<%
						if (funck_new) {
					%>

					<input type="button" value="新增群組" class="btn"
						onclick="go_log('<%=fid%>','1');go_edit('0','newgroup.jsp');">
			...
			<div class="row">
				...
				<div class="column" data-label="群組說明"><%=rst.showData("gdesc", i)%></div>
				<div class="column" data-label="編輯群組">
					...
					<input type="button" id="s1" style="display: none" value="新增群組"
						class="btn"
						onclick="go_log('<%=fid%>','1');go_edit('0','newgroup.jsp');">
					<%
						if (funck_edit) {
					%>
					<input type="button" value="修改群組" class="btn"
						onclick="go_log('<%=fid%>','2');go_edit('<%=rst.showData("gid", i)%>','newgroup.jsp');">
					<%
						}
					%>
					<%
						if (funck_del) {
					%>
					<input type="button" value="刪除群組" class="btn"
						onclick="go_log('<%=fid%>','3');del_group('<%=rst.showData("gid", i)%>','1');">
					<%
						}
					%>
