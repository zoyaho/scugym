...
$('#ge').datetimepicker({
	...
	<section id="content" class="column-center">
		<div id="search">
			<form class="pure-form">
				<fieldset>
					<legend>查尋停權紀錄 </legend>
					<br> <label for="起始日期">停權日期(起):</label> 
					<input type="text" name="sdate" id="gs" class="pop4font" value="" /> 
					<label for="結束日期">停權日期(迄):</label> 
					...
					<%
						if (funck_new) {
					%>
					<input type="button" class="btn" value="新增停權"
						onclick="go_log('<%=fid%>','1');edit_news('0','newPenalty.jsp');">
					<%
						}
					%>
  ...
  $("#send_search").click(function(event){
	    go_log('<%=fid%>', '0');
					...
					{
						if (a != '' && b != '') {
							//alert("list_point.jsp?gs="+a+"&ge="+b+"&gk="+c+"&gv="+d+"&gl="+e);
							try
							{
							...
							$("#middle1").load("list_penalty.jsp?gs=" + a + "&ge=" + b
	...
	<div id="middle1">
		<jsp:include page="list_penalty.jsp"></jsp:include>
	</div>
	<%
		} else {
	%>
